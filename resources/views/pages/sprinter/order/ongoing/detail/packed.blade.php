...
@section("actions")
    <div class="py-3">
        <form method="POST" action="{{route('sprinter.order.ongoing.to.shipment',\Illuminate\Support\Facades\Route::current()->parameters)}}">
            @csrf
            <x-form.button isSubmit fullWidth type="{{\App\View\Components\Form\Button::TYPE_INFO}}">
                {{__('messages.sprinter.form.submit.to_shipment_partner')}}
            </x-form.button>
        </form>
    </div>
@endsection
